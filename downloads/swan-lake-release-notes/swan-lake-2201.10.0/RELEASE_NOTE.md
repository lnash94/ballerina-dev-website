...
#### OpenAPI tool
-  Added support to generate a mock client for OAS if including the example details. To generate mock client use `--mock` flag along with openAPI CLI client generation.
  ```
  $ bal openapi -i <yml file> --mode client --mock
  ```

-  Provided the option to generate a single bal file for client/service. To generate single file use `--single-file` flag along with openAPI CLI client/service generation.
  ```
  $ bal openapi -i <yml file> --mode <client/service> --single-file
  ```

-  Added support for generating Ballerina service object type from given OAS. To generate service contract object type file use `--with-service-contract` flag along with openAPI CLI service generation.
  ```
...
ex:
 ...
 ```
  ...
  ```
- Provided option to generate Ballerina client/service according to Ballerina friendly naming convention. To enable this feature use `--use-sanitized-oas` flag along with client/service CLI code generation.
  ```
...
#### OpenAPI tool
- Added support for Server Sent Events (SSE) support for Ballerina Client generation.
- Added improvements for supporting reference JSON example file path for the examples field in `@openapi:ResourceInfo` annotation.
- Added support request body example mapping for `@openapi:ResourceInfo` annotation in OAS generation.
